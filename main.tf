...
resource "aws_ecs_task_definition" "my_first_task" {
  family                   = "rilCap2TaskDef2" # Naming our first task
  container_definitions    = <<DEFINITION
  [
    {
      "name": "rilCap2TaskDef2",
...
resource "aws_iam_role" "ecsTaskExecutionRole" {
  name               = "rilCap2EcsTaskExecutionRole"
...
resource "aws_alb" "application_load_balancer" {
  name               = "ril-cap2-lb-tf" # Naming our load balancer
...
resource "aws_ecs_service" "my_first_service" {
  name            = "ril-cap2-service"                             # Naming our first service
...
resource "null_resource" "docker_packaging" {
	  ...
	  provisioner "local-exec" {
	    ...
	    docker build -t ril-cap2-ecr .
	    docker tag ril-cap2-ecr:latest 962804699607.dkr.ecr.us-west-2.amazonaws.com/ril-cap2-ecr:latest
	    docker push 962804699607.dkr.ecr.us-west-2.amazonaws.com/ril-cap2-ecr:latest
	    EOF
	  }
